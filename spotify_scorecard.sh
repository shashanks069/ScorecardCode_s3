...
do
        ...
        M=`date -d "${DAY}" '+%m'`
        D=`date -d "${DAY}" '+%d'`
        for CN in `aws s3 ls ${RAW_S3_PATH}/year=${Y}/month=${M}/day=${DAY}/ | grep -v 'folder' | awk '{print $2}' | grep -o '...$' | cut -b1-2`;
        do
                CN_TIME=""
                REPORT_DATE=$(date -d "$start + 0 day" +"%Y-%m-%d")
                RUN_DATE=$(date -d "$start + 2 day" +"%Y-%m-%d")
                CN_TIME=`aws s3 ls ${RAW_S3_PATH}/year=${Y}/month=${M}/day=${DAY}/country_code=${CN}/ | awk 'FNR==1 {print $1,$2}'`
		CN_DATE=`aws s3 ls ${RAW_S3_PATH}/year=${Y}/month=${M}/day=${DAY}/country_code=${CN}/ | awk 'FNR==1 {print $1}'`
